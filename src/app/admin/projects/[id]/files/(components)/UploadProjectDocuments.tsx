...
import { addFilesToProject } from '@/actions/projects';
...
export default function UploadProjectDocuments({ setProject }: Props) {
  ...
  const UploadProjectFiles = async () => {
    ...
    if (files.length > 0) {
      for (const file of files) {
        ...
        const res = await edgestore.cavoaDocuments.upload({ file, options: { manualFileName: fileName } });
        urls.push({ name: res.url, url: res.url });
      }
    }
    try {
      ...
      toast({
        title: 'Algo sucedio!',
        description: 'Error al actualizar los documentos',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="grid gap-2 grid-cols-1">
      <Label className="text-lg font-bold my-4">Carga nuevos documentos</Label>
